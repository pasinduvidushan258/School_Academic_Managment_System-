...
namespace Project
{
    public partial class add_mark_from : Form
    {
        ...
        public add_mark_from(int classID, int term)
        {
            InitializeComponent();
        ...
        {
            string query = "SELECT SubjectID FROM teacher_allocations WHERE TeacherID = @TeacherID AND ClassID = @ClassID LIMIT 1";
            using (MySqlConnection conn = new MySqlConnection(connectionString))
            {
                using (MySqlCommand cmd = new MySqlCommand(query, conn))
                {
                    ...
                    try
                    {
                        ...
                        if (res != null) _subjectID = Convert.ToInt32(res);
                    }
                    catch (Exception ex)
                    {
                        MessageBox.Show("Subject check madooralli dosha ide: " + ex.Message);
                    }
                }
            }
        }
        ...
        {
            // Left join pulls the student data list and overlays editable mark lines safely
            string query = @"
                SELECT 
            ...
            using (MySqlConnection conn = new MySqlConnection(connectionString))
            {
                ...
                {
                    MessageBox.Show("Marks grid not load: " + ex.Message);
                }
            }
        }


        private void add_mark_from_Load(object sender, EventArgs e)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(connectionString))
            {
                try
                {
                    ...
                    foreach (DataGridViewRow row in dataGridView_marksAdd.Rows)
                    {
                        ...
                        {
                            MessageBox.Show($"{studentNo} ge thappaada marks haakideera! (0 - 100 matra aagidabeku)", "Validation Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                            return;
                        }
                    ...
                    MessageBox.Show("Marks add Successfully!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
